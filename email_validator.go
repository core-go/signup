...
func NewEmailValidator(passwordRequired bool, paswordExp string, fields ...string) *EmailValidator {
	...
	return &EmailValidator{PasswordRequired: passwordRequired, Regexp: regExp, requiredFields: requiredFields, modelIndexes: modelIndexes}
}

func (v *EmailValidator) Validate(ctx context.Context, user SignUpInfo) (msgs []ErrorMessage, err error) {
	...
	if len(user.Contact) > 0 {
		emailMsg := CheckEmail("contact", user.Contact)
		if emailMsg != nil {
